    ...
    <>
      <div className="flex flex-col lg:flex-row">
          <div className="flex items-center gap-5 flex-wrap">
              {
                recipes.map((recipe,i) => <Recipe key={i} recipe={recipe} handleCookBtn={handleCookBtn}></Recipe>)
              }
                  ...
                  <div className='bg-[#2828284a]'>
                     {
                      cook.map((recipe,index) => (
                          <div className='flex items-center gap-10 m-auto w-[100%] justify-center' key={index}>
                            <p>{index + 1}</p>
                            <p>{recipe.recipe_name}</p>
                            <p className='mr-5'>{recipe.preparing_time}</p>
                            <p>{recipe.calories}</p>
                            <button className='mt-0 py-2 px-5 bg-[#0BE58A] text-black rounded-3xl border-none  font-extrabold cursor-pointer hover:bg-slate-200 hover:text-black'>Preparing</button>
                          </div>
                        )
                      )
                     } 
